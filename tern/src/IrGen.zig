...
pub const SymbolTable = struct {
    ...
    pub fn enterScope(self: *SymbolTable, allocator: std.mem.Allocator) ErrorSet!void {
        ...
        if (self.current_scope) |this_scope| {
            const current_scope_object: *Scope = self.getScope(this_scope) catch return error.OutOfRange;
    ...
    pub const AddInfo = struct {
        typ: ir.TaggedInstructionIndex(ir.Instruction.Type),
        mutable: bool,
        instruction: ir.InstructionIndex = .none,
    };
    pub fn add(self: *SymbolTable, allocator: std.mem.Allocator, name: []const u8, info: AddInfo) ErrorSet!void {
        ...
        var scope = &self.scopes.items[@intFromEnum(self.current_scope orelse return error.OutOfRange)];
    ...
    pub fn lookup(self: *SymbolTable, name: []const u8) ?Symbol {
        // check core
        if (primitive_lookup.get(name)) |symbol| {
            return symbol;
        }
    ...
    pub fn getScope(self: *SymbolTable, index: ScopeIndex) ErrorSet!*Scope {
...
const Primitives = struct {
    fn primitiveSymbol(comptime name: []const u8, instruction: ir.InstructionIndex) struct { []const u8, Symbol } {
        return .{ name, .{
            .name = name,
            .typ = .type_type,
            .mutable = false,
    ...
    pub const lookup = std.StaticStringMap(Symbol).initComptime(.{
        primitiveSymbol("i8", .type_i8),
        primitiveSymbol("i16", .type_i16),
        primitiveSymbol("i32", .type_i32),
        primitiveSymbol("i64", .type_i64),
        primitiveSymbol("u8", .type_u8),
        primitiveSymbol("u16", .type_u16),
        primitiveSymbol("u32", .type_u32),
        primitiveSymbol("u64", .type_u64),
        primitiveSymbol("f32", .type_f32),
        primitiveSymbol("f64", .type_f64),
    });
};
pub const primitive_lookup = Primitives.lookup;
